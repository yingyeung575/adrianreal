            ...
            <div class="articles">
                    <div class="container max-width-adaptive-lg">
                      <div class="text-center margin-bottom-xl">
                            <h1 v-show='category.slug == $route.params.slug' v-for="(category,idx) in categories" :key='idx'>{{ category.name }}</h1> 
                        ...
                        <article class="story story--featured" v-show='idx==0' v-for="(article, idx) in filteredarticles" v-bind:key="idx">
                          ...
                          <div class="story__content">
                            ...
                            <div class="story__author margin-top-sm">
                              ...
                              <div>
                                <address v-show='article.author' class="story__author-name"><span rel="author">{{ article.author.name }}</span></address>
                      ...
                      <nav v-show='maxpage!=1 && maxpage!=0' class="pagination margin-top-xxl" aria-label="Pagination">
                        <ol class="pagination__list flex flex-wrap flex-gap-xxs justify-center">
                          <li>
                            <a v-show='currentPage!=1' v-bind:href="backlink" class="pagination__item" aria-label="Go to previous page">
                              <svg class="icon margin-right-xxxs" aria-hidden="true" viewBox="0 0 16 16"><title>Previous</title><g stroke-width="1" stroke="currentColor"><polyline fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" points="9.5,3.5 5,8 9.5,12.5 "></polyline></g></svg>
                              <span>上一頁</span>
                            </a>
                          </li>
                  
                        <!--  <li v-show='maxpage - $route.query.page < 2' class="display@sm" aria-hidden="true">
                            <span class="pagination__item pagination__item--ellipsis">...</span>
                          </li> -->

                          <span class="display@sm" v-for='id in maxpage' v-bind:key="id">
                          
                          <li v-show='id - currentPage>= 1 && id - currentPage < 3' >
                            <a :href="gopage(id)" class="pagination__item">{{id}}</a>
                          </li>
                          <li v-show='id == currentPage' >
                            <a :href="gopage(id)" class="pagination__item pagination__item--selected">{{id}}</a>
                          </li>
                          </span>

                          <li v-show='maxpage - currentPage > 2' class="display@sm" aria-hidden="true">
                            <span class="pagination__item pagination__item--ellipsis">...</span>
                          </li>
                          ...
                          <li>
                            <a v-show='currentPage!=maxpage' v-bind:href="nextlink" class="pagination__item" aria-label="Go to next page">
                              <span>下一頁</span>
...
export default {  
  ...
  async asyncData({ $axios,query,params }) {
    ...
    const articles = await $axios.$get(process.env.backendurl+'articles?categories.slug='+params.slug+'&_limit=10&_start='+ thepage + '&_sort=date:DESC')
    const categories = await $axios.$get(process.env.backendurl+'categories')
    const prenoarticles = await $axios.$get(process.env.backendurl+'articles?categories.slug='+params.slug)
    const noarticles = prenoarticles.length
    return { articles,noarticles,categories }
  },
  methods:{
    gopage( para ){
      return '/category/' + this.$route.params.slug + '/' + para
    },
  },
  computed: {
    ...
    backlink(){
      ...
      return '/category/' + this.$route.params.slug + '/' + backpageno
    },
    nextlink(){
      let nextpageno = 2
      if (this.$route.params.page)
        nextpageno = parseInt(this.$route.params.page) + 1
      return '/category/' + this.$route.params.slug + '/' + nextpageno
    },
    maxpage(){
