      ...
      <div v-if='!selectedsubject' class="modal modal--select js-modal modal--is-visible modalblock" id="modal-select" role="dialog" aria-labelledby="modal-select-label" data-animation="on">
                ...
                <div class="vvcenter max-width-xs text-center padding-md" role="alertdialog" tabindex="-1" aria-labelledby="modal-select-title" aria-describedby="">
                  ...
                  <ul class="margin-top-xl text-center full-screen-select__list js-full-screen-select__list" role="listbox">
                    <p class='noresult padding-top-sm padding-left-sm text-md' v-if='loading'>請稍等。資料更新中....<br><br><img width='40' class='loadingicon' src='/img/loading.svg'></p>
                    <li v-for='(subject, idx) in showsubjects' v-bind:key="idx">
                      <nuxt-link :to="'/school-type/international-one?subject=' + subject.slug" v-if='idx==0' class="full-screen-select__option full-screen-select__option--selected" role="option">{{ subject.name }}</nuxt-link>
                      <nuxt-link :to="'/school-type/international-one?subject=' + subject.slug" v-if='idx!=0' class="full-screen-select__option" role="option">{{ subject.name }}</nuxt-link>
                    </li>
...
export default {
  ...
  /*  const subjects = await $axios.$get(process.env.backendurl+'subjects/findmin?isinternation1_eq=true')
    return { subjects } */
  },
  async created(){

    this.subjects =  await this.$axios.$get(process.env.backendurl+'subjects-inter-1-s')
